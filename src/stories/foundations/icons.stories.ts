...
export const AllIcons: StoryFn = () => ({
  ...
  setup () {
    ...
    const eosIcons = Object.entries(EOS_ICONS).map(([key, icon]) => [`EOS_ICONS.${key}`, icon])
    const dataUrlIcons = Object.entries(ICONS_DATA_URL).map(([key, icon]) => [`ICONS_DATA_URL.${key}`, icon])
    const icons = [...mdiIcons, ...riIcons, ...eosIcons, ...dataUrlIcons]
    return { icons }
  },
  template: `
...
export const EosIcons: StoryFn = () => ({
  components: { AvIcon },
  setup () {
    return { icons: Object.entries(EOS_ICONS).map(([key, icon]) => [`EOS_ICONS.${key}`, icon]) }
  },
  template: `
    <div style="display: flex; flex-wrap: wrap; gap: var(--spacing-sm);">
      <div 
